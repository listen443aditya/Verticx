...
interface RaiseComplaintModalPrincipalProps {
  student: Student;
  onClose: () => void;
  onSubmit: () => void;
}

const RaiseComplaintModalPrincipal: React.FC<
  RaiseComplaintModalPrincipalProps
> = ({ student, onClose, onSubmit }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // FIX: The type definition demands all properties. We will provide them explicitly.
      // The API must be designed to accept this full object.
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
      <Card className="w-full max-w-lg">
        ...
        <p className="text-sm text-text-secondary-dark mb-4">
          This complaint will be logged and visible to the student and their
          parent.
        </p>
...
const GrievanceDiscipline: React.FC = () => {
  ...
  const classMap = useMemo(() => {
    return new Map(
      classes.map((c) => [c.id, `Grade ${c.gradeLevel} - ${c.section}`])
    );
  }, [classes]);
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-text-primary-dark mb-6">
        Raise a Disciplinary Complaint
      </h1>
      <Card>
        ...
        <p className="text-sm text-text-secondary-dark mb-4">
          Select a student to raise a formal complaint regarding discipline or
          other issues. This will be logged and made visible to the student and
          their parent.
        </p>
        <Input
        ...
        />
        {complaintStatus && (
          <p className="text-center text-green-600 mb-4">{complaintStatus}</p>
        )}
        {loading ? (
