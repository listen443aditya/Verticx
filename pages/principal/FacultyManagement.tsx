...
const CredentialsModal: React.FC<{
  ...
  <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
    <Card className="w-full max-w-md">
      <h2 className="text-xl font-bold text-brand-accent mb-2">
        Account Created Successfully!
      </h2>
      <p className="text-text-secondary-dark mb-4">
        The staff account has been created. An SMS with these credentials has
        been sent to their registered phone number.
      </p>
...
const FacultyManagement: React.FC = () => {
  ...
  const [suspendingStaff, setSuspendingStaff] = useState<StaffMember | null>(
    null
  );
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    // FIX: Renamed methods to match refactored service and removed branchId argument.
  ...
  const handleReject = async (applicationId: string) => {
    if (!user) return;
    setIsActionLoading(true);
    // FIX: rejectFacultyApplication now takes only 1 argument.
    await principalApiService.rejectFacultyApplication(applicationId);
    triggerRefresh();
    setIsActionLoading(false);
  };
  ...
  const getStatusChipClass = (
    status: "active" | "suspended" | undefined | string
  ) => {
    switch (status) {
      case "active":
    ...
    <div>
              ...
              <tbody>
                {staff.map((staffMember) => (
                  <tr key={staffMember.id} className="border-b">
                    ...
                    <td className="p-2">
                      <span
                        className={`px-2 py-1 text-xs font-semibold rounded-full ${getStatusChipClass(
      ...
      {suspendingStaff && (
        <ConfirmationModal
          ...
          title={`Confirm ${
            suspendingStaff.status === "active" ? "Suspension" : "Reinstatement"
          }`}
          message={
            <>
              Are you sure you want to{" "}
              {suspendingStaff.status === "active" ? "suspend" : "reinstate"}{" "}
      ...
      {deletingStaff && (
        <ConfirmationModal
          ...
          message={
            <>
              Are you sure you want to permanently delete {deletingStaff.name}?
