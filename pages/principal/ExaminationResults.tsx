...
const ExaminationResults: React.FC = () => {
  ...
  const [publishingExam, setPublishingExam] = useState<Examination | null>(
    null
  );
  const [showSmsModalFor, setShowSmsModalFor] = useState<Examination | null>(
    null
  );
  ...
  const fetchData = useCallback(async () => {
    if (!user) return;
    setLoading(true);
    const exams = await apiService.getExaminationsWithResultStatus();
              ...
              <tbody>
                {examinations.map((exam) => (
                  <tr key={exam.id} className="border-b hover:bg-slate-50">
                    ...
                    <td className="p-4">
                      <span
                        className={`px-2 py-1 text-xs font-semibold rounded-full ${getResultStatusChip(
                          exam.resultStatus
                        )}`}
                      >
                        {exam.resultStatus}
        ...
        <ConfirmationModal
          ...
          message={
            <>
              ...
              <strong>{publishingExam.name}</strong>? This action will make all
              marks for this exam visible to students and parents and cannot be
              undone.
            </>
          }
          confirmText="Publish"
