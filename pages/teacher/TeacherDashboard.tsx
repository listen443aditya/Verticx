...
//         if (!user?.branchId || !user?.id) return; // Guard against missing user info
//         setLoading(true);

//         try {
//           // This Promise.all call is now cleaner and more secure
//           const [dashboardData, transportData, branchData] = await Promise.all([
// apiService.getTeacherDashboardData(), // It no longer needs the teacherId passed from the frontend
//             apiService.getMyTransportDetails(), // <--- CALL THE NEW, CORRECT METHOD
...
// --- Sub-component for the Weekly Schedule ---
// This is now highly efficient, performing fast lookups on pre-fetched data.
const WeeklySchedule: React.FC<{
  schedule: TimetableSlot[],
  allClasses: Pick<SchoolClass, 'id' | 'gradeLevel' | 'section'>[],
  allSubjects: Pick<Subject, 'id' | 'name'>[]
}> = ({ schedule, allClasses, allSubjects }) => {
    const today = new Date().toLocaleString('en-us', { weekday: 'long' }) as any;
    const [selectedDay, setSelectedDay] = React.useState<'Monday' | 'Tuesday' | 'Wednesday' | 'Thursday' | 'Friday'>(days.includes(today) ? today : 'Monday');
    ...
    return (
        <div>
            ...
            <div className="space-y-3 mt-4">
                {daySchedule.length > 0 ? daySchedule.map(slot => {
                    // THE FIX: Fast, synchronous lookups. No more slow API calls.
                    const sClass = allClasses.find(c => c.id === slot.classId);
...
const StatCard: React.FC<{ title: string; value: string; icon: React.ReactNode; onClick?: () => void; }> = ({ title, value, icon, onClick }) => (
    <Card className={`flex items-center p-4 ${onClick ? 'cursor-pointer hover:bg-slate-50 transition-colors' : ''}`} onClick={onClick}>
        <div className="p-3 mr-4 text-brand-secondary bg-brand-primary/10 rounded-full">{icon}</div>
        <div>
            <p className="text-sm font-medium text-text-secondary-dark">{title}</p>
            <p className="text-2xl font-semibold text-text-primary-dark">{value}</p>
...
const TeacherDashboard: React.FC = () => {
    ...
    const [principal, setPrincipal] = React.useState<User | null>(null);
    const [transportDetails, setTransportDetails] = React.useState<{ route: TransportRoute, stop: BusStop } | null>(null);
    ...
    React.useEffect(() => {
        const fetchDashboardData = async () => {
            if (!user?.branchId) {
                setError("User branch information is missing.");
                setLoading(false);
                return;
            }
            ...
            try {
                ...
                // One swift transaction for all required data
                const [dashboardData, transportData, branchData] = await Promise.all([
                    apiService.getTeacherDashboardData(), // No teacherId needed; backend gets it from the token
                    apiService.getMyTransportDetails(), // Uses the new, secure teacher-specific endpoint
                    sharedApiService.getBranchById(user.branchId)
                ]);
    ...
    if (!data) return <div className="text-center p-8">No dashboard data available.</div>;
    ...
    const { 
        ...
        library
    } = data as TeacherDashboardData;

    // Use runtime-safe fallbacks for optional properties that may not exist on the backend DTO.
    const allBranchClasses: Pick<SchoolClass, 'id' | 'gradeLevel' | 'section'>[] = (data as any).allBranchClasses ?? [];
    const allBranchSubjects: Pick<Subject, 'id' | 'name'>[] = (data as any).allBranchSubjects ?? [];
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-6">
                    <Card>
                        ...
                        <WeeklySchedule 
                            ...
                            allClasses={allBranchClasses}
                            allSubjects={allBranchSubjects}
                        />
                    </Card>
                    <Card>
                        <h2 className="text-xl font-semibold text-text-primary-dark mb-4">Class Performance Overview</h2>
                ...
                <div className="space-y-6">
                    ...
                    <Card>
                        <h2 className="text-xl font-semibold mb-4 text-red-800">At-Risk Students</h2>
                        <div className="space-y-2">
                             {atRiskStudents.length > 0 ? atRiskStudents.map(s => (
                                <div key={s.studentId} className="bg-red-50 p-2 rounded-md text-sm">
                                    <p className="font-medium text-red-800">{s.studentName}</p>
             ...
             <AssignHomeworkModal
                isOpen={isHomeworkModalOpen}
                onClose={() => setIsHomeworkModalOpen(false)}
                onSave={() => setIsHomeworkModalOpen(false)} // Placeholder action
            />
             <div className="mt-6">
                <ContactCard branch={branch || undefined} principalName={principal?.name} />
            </div>
