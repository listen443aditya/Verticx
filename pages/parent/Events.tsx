// pages/parent/Events.tsx
import React, { useState, useEffect, useCallback, useMemo } from "react";
import { useAuth } from "../../hooks/useAuth.ts";
// import the class directly and instantiate
...
const Events: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      // NOTE: If your ParentApiService uses a different method name for events,
      // change getSchoolEvents -> the actual method (e.g. getParentEvents or getEvents).
      const data = await apiService.getSchoolEvents(user.branchId);
      // only show approved events visible to parents/students/all
      const filtered = data.filter(
        (ev: SchoolEvent) =>
          ev.status === "Approved" &&
          (ev.audience.includes("All") ||
            ev.audience.includes("Parents") ||
            ev.audience.includes("Students"))
      );
      setEvents(filtered);
    } catch (err) {
  ...
  const startingDayOfWeek = (firstDayOfMonth.getDay() + 6) % 7; // monday-first

  const eventsByDate = useMemo(() => {
    const map = new Map<string, SchoolEvent[]>();
    events.forEach((ev: SchoolEvent) => {
