...
const GradeBook: React.FC = () => {
  ...
  useEffect(() => {
    const fetchChildren = async () => {
      ...
      // explicit types for callbacks to avoid implicit any
      const childrenPromises: Promise<(Student | undefined)[]> = Promise.all(
        user.childrenIds.map((id: string) =>
          apiService
            .getStudentProfileDetails(id)
            .then((p: StudentProfile | null) => p?.student)
        )
      );

      const childrenData = (await childrenPromises).filter(
        (s): s is Student => !!s
      );
