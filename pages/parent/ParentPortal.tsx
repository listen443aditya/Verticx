...
const ParentSidebarContent: React.FC<{
  onClose?: () => void;
  onOpenProfile: () => void;
  ...
  return (
    <aside className="w-64 bg-slate-800 text-slate-100 flex flex-col p-4 border-r border-slate-700 h-full">
      ...
      <nav className="flex-1 space-y-2 overflow-y-auto pr-2">
        <NavLink to="/parent/dashboard" className={navLinkClasses}>
        ...
        <NavLink to="/parent/grades" className={navLinkClasses}>
          <div className="w-4 h-4 mr-3 flex items-center justify-center flex-shrink-0">
            <AcademicsIcon className="w-full h-full" />
          </div>
          <span className="truncate min-w-0">Grade Book</span>
        </NavLink>
        <NavLink to="/parent/fees" className={navLinkClasses}>
          <div className="w-4 h-4 mr-3 flex items-center justify-center flex-shrink-0">
            <BanknoteIcon className="w-full h-full" />
          </div>
          <span className="truncate min-w-0">Fee Management</span>
        </NavLink>
        {user?.enabledFeatures?.parent_complaints && (
          <NavLink to="/parent/complaints" className={navLinkClasses}>
            <div className="w-4 h-4 mr-3 flex items-center justify-center flex-shrink-0">
        ...
        {user?.enabledFeatures?.parent_contact_teacher && (
          <NavLink to="/parent/contact" className={navLinkClasses}>
            <div className="w-4 h-4 mr-3 flex items-center justify-center flex-shrink-0">
              <CommunicationIcon className="w-full h-full" />
            </div>
            <span className="truncate min-w-0">Contact Teacher</span>
          </NavLink>
        )}
        <NavLink to="/parent/events" className={navLinkClasses}>
          <div className="w-4 h-4 mr-3 flex items-center justify-center flex-shrink-0">
