// pages/parent/FeeManagement.tsx
import React, { useEffect, useState, useCallback } from "react";
import { useAuth } from "../../hooks/useAuth.ts";
// instantiate the service class directly
...
const FeeManagement: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      // <-- NOTE: no user.id param. Backend identifies parent via auth token.
  ...
  if (!data || !selectedChildData || !user)
    return <Card>No student data found.</Card>;

  const { profile, fees, feeHistory, branch } = selectedChildData as any; // keep 'any' if types are slightly different in your types.ts
  const {
  ...
  return (
    <div className="space-y-6">
          ...
          <div>
            ...
            >
              {data.childrenData.map((child, index) => (
                <option key={child.profile.id} value={index}>
                  {child.profile.name}
                </option>
              ))}
            </select>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-6">
          <Card>
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between">
                ...
                <strong>
                  {fees.totalAnnualFee?.toLocaleString() ?? "N/A"}
                </strong>
              </div>
              <div className="flex justify-between">
                <span className="text-text-secondary-dark">Total Paid:</span>{" "}
                <strong className="text-green-600">
                  {fees.totalPaid?.toLocaleString() ?? "N/A"}
                </strong>
              </div>
              <div className="flex justify-between text-base pt-2 border-t mt-2">
                ...
                <strong className="text-red-500 text-lg">
                  {fees.totalOutstanding?.toLocaleString() ?? 0}
                </strong>
              </div>
            </div>
          ...
          <Card>
            <h2 className="text-xl font-semibold mb-4 flex items-center gap-2">
              <BanknoteIcon /> School Bank Details
            </h2>
            {bankAccountNumber && bankAccountHolderName ? (
              <div className="space-y-2 text-sm bg-slate-50 p-3 rounded-lg">
                <div className="flex justify-between">
                  ...
                  <strong>{bankAccountHolderName}</strong>
                </div>
                <div className="flex justify-between">
                  <span className="text-text-secondary-dark">
                    Account Number:
                ...
                <div className="flex justify-between">
                  <span className="text-text-secondary-dark">IFSC Code:</span>{" "}
                  <strong>{bankIfscCode}</strong>
                </div>
                <div className="flex justify-between">
                  <span className="text-text-secondary-dark">Branch Name:</span>{" "}
                  <strong>{bankBranchName}</strong>
                </div>
              </div>
            ) : (
              <p className="text-sm text-center text-text-secondary-dark p-4">
                Bank details have not been configured by the school.
              </p>
            )}
          </Card>
        </div>
        ...
        <div className="lg:col-span-2">
          <Card>
            ...
            {fees.monthlyDues.filter((due: MonthlyDue) => due.status !== "Paid")
              .length > 0 ? (
              <div className="space-y-3">
                {fees.monthlyDues
                  .filter((due: MonthlyDue) => due.status !== "Paid")
                  .map((due: MonthlyDue) => (
                    <div
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-text-secondary-dark">
                          Total: {due.total.toLocaleString()} | Paid:{" "}
      ...
      {isPayFeesModalOpen && selectedChildData && (
        <PayFeesModal
          isOpen={isPayFeesModalOpen}
          onClose={() => setIsPayFeesModalOpen(false)}
          student={selectedChildData.student ?? selectedChildData.profile}
