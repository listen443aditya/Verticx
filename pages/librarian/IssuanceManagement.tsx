...
import type { HydratedBookIssuance, User, LibraryBook, Student, Teacher } from '../../types.ts';
...
const IssuanceManagement: React.FC = () => {
    ...
    const [memberSuggestions, setMemberSuggestions] = useState<(Student | Teacher)[]>([]);


    const fetchData = useCallback(async () => {
        if (!user?.branchId) return;
        setLoading(true);
        const [issuanceData, bookData, studentData, teacherData] = await Promise.all([
    ...
    const handleIssue = async (e: React.FormEvent) => {
        ...
        try {
            const bookToIssue = allBooks.find(b => b.isbn === bookIdentifier) || allBooks.find(b => b.id === bookIdentifier);
            const bookIdToSubmit = bookToIssue ? bookToIssue.id : bookIdentifier;

            const response = await apiService.issueBookByIsbnOrId(user.branchId, bookIdToSubmit, memberId, dueDate, Number(finePerDay));
            setActionStatus({ message: `Successfully issued "${response.bookTitle}" to ${response.memberName}.`, type: 'success' });
            setBookIdentifier('');
            setMemberId('');
            await fetchData();
        } catch (error: any) {
    ...
    const handleBookIdentifierChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const currentlyIssued = useMemo(() => issuances.filter(i => !i.returnedDate).sort((a,b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime()), [issuances]);
    const history = useMemo(() => issuances.filter(i => i.returnedDate).sort((a,b) => new Date(b.returnedDate!).getTime() - new Date(a.issuedDate).getTime()), [issuances]);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <Card className="lg:col-span-1">
                    ...
                    <form onSubmit={handleIssue} className="space-y-4">
                            ...
                            {bookSuggestions.length > 0 && (
                                <ul className="absolute z-20 w-full bg-white border border-slate-300 rounded-md mt-1 shadow-lg max-h-40 overflow-y-auto">
                                    {bookSuggestions.map(book => (
                                        <li key={book.id} onClick={() => selectBook(book)} className="px-4 py-2 hover:bg-slate-100 cursor-pointer text-sm">
                            ...
                            {memberSuggestions.length > 0 && (
                                <ul className="absolute z-10 w-full bg-white border border-slate-300 rounded-md mt-1 shadow-lg max-h-40 overflow-y-auto">
                                    {memberSuggestions.map(member => (
                                        <li key={member.id} onClick={() => selectMember(member)} className="px-4 py-2 hover:bg-slate-100 cursor-pointer text-sm">
                                            <p className="font-semibold">{member.name}</p>
                                            <p className="text-xs text-slate-500">{member.id} - {(member as Student).gradeLevel ? 'Student' : 'Teacher'}</p>
                                        </li>
                                    ))}
                                </ul>
                            )}
                        ...
                        {actionStatus.message && (
                            <p className={`text-sm text-center p-2 rounded ${actionStatus.type === 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                {actionStatus.message}
                            </p>
                        )}
                ...
                <Card className="lg:col-span-2">
                    <div className="flex border-b border-slate-200 mb-4">
                        <button className={tabButtonClasses(activeTab === 'issued')} onClick={() => setActiveTab('issued')}>Currently Issued ({currentlyIssued.length})</button>
                        <button className={tabButtonClasses(activeTab === 'history')} onClick={() => setActiveTab('history')}>History ({history.length})</button>
                    </div>
                        ...
                        <table className="w-full text-left text-sm">
                            <thead className="sticky top-0 bg-surface-dark">
                                <tr className="border-b">
                                    ...
                                    <th className="p-2">{activeTab === 'issued' ? 'Due Date' : 'Issued / Returned'}</th>
                                    <th className="p-2"></th>
                                </tr>
                            </thead>
                            <tbody>
                                {(activeTab === 'issued' ? currentlyIssued : history).map(item => {
                                    const isOverdue = !item.returnedDate && new Date(item.dueDate) < new Date();
                                    return (
                                        <tr key={item.id} className={`border-b ${isOverdue ? 'bg-red-50' : 'hover:bg-slate-50'}`}>
                                            <td className="p-2 font-medium">{item.bookTitle}</td>
                                            <td className="p-2">{item.memberName} <br/><span className="text-xs text-slate-500">{item.memberDetails}</span></td>
                                            <td className={`p-2 font-semibold ${isOverdue ? 'text-red-600' : ''}`}>
                                                {activeTab === 'issued' 
                                                    ? new Date(item.dueDate).toLocaleDateString()
                                                    : `${new Date(item.issuedDate).toLocaleDateString()} - ${new Date(item.returnedDate!).toLocaleDateString()}`
                                                }
                                            </td>
