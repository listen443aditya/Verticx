...
import { LibrarianApiService, SharedApiService } from "../../services";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        ...
        <StatCard
          title="Books Issued"
          value={summary.issuedBooks.toLocaleString()}
          icon={<MoveIcon className="w-5 h-5" />}
        />
        <StatCard
          title="Books Overdue"
          value={summary.overdueBooks.toLocaleString()}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              recentActivity.map((item) => (
                <div
                  ...
                  className={`p-3 rounded-lg flex justify-between items-center ${
                    item.returnedDate ? "bg-slate-100" : "bg-blue-50"
                  }`}
                >
                  <div>
                    ...
                    <p className="text-xs text-text-secondary-dark">
                      {new Date(
                        item.returnedDate || item.issuedDate
                      ).toLocaleString()}
                    </p>
                  </div>
                  <span
              ...
              overdueList.map((item) => (
                ...
                >
                  ...
                  <p className="text-xs text-red-600">
                    Issued to: {item.memberName} ({item.memberDetails})
                  </p>
                  <p className="text-xs text-red-600">
        ...
        <Card className="lg:col-span-1">
          ...
          <div className="space-y-3 max-h-96 overflow-y-auto">
            {classIssuanceSummary.length > 0 ? (
              classIssuanceSummary.map((summary) => (
                <details
                  key={summary.classId}
                  ...
                  <ul className="mt-2 pt-2 border-t text-xs list-disc pl-5">
                    {summary.studentsWithBooks.map((s) => (
                      <li key={s.studentName}>
                        {s.studentName} ({s.bookCount} book
                        {s.bookCount > 1 ? "s" : ""})
                      </li>
                    ))}
                  </ul>
              ...
              <p className="text-center text-text-secondary-dark p-4">
                No books currently issued to students.
              </p>
            )}
          </div>
        </Card>
