...
const BookFormModal: React.FC<{
    ...
    const [formData, setFormData] = useState<Partial<LibraryBook>>({
        title: '', author: '', isbn: '', totalCopies: 1, price: 0, ...book
    });
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setFormData(prev => ({ ...prev, [name]: (name === 'totalCopies' || name === 'price') ? Number(value) : value }));
    };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
            <Card className="w-full max-w-lg">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <Input label="Book Title" name="title" value={formData.title} onChange={handleChange} required />
                    ...
                    />
                    <Input label="Total Copies" name="totalCopies" type="number" min="1" value={formData.totalCopies} onChange={handleChange} required />
                    <Input label="Price" name="price" type="number" min="0" step="0.01" value={formData.price || ''} onChange={handleChange} />
                    <Input label="Upload PDF (Optional)" type="file" name="pdfFile" accept=".pdf" onChange={handleFileChange} />
...
const BookManagement: React.FC = () => {
    ...
    const handleSave = async (bookData: Partial<LibraryBook>, pdfFile: File | null) => {
        if (!user?.branchId) return;
    ...
    return (
        <div>
                            ...
                            <tbody>
                                {filteredBooks.map(book => (
                                    <tr key={book.id} className="border-b hover:bg-slate-50">
                                        ...
                                        <td className="p-4 text-center font-semibold">{book.availableCopies} / {book.totalCopies}</td>
                                        <td className="p-4 text-right font-semibold">{book.price?.toFixed(2) || 'N/A'}</td>
                                        <td className="p-4 text-right">
                                            <div className="flex gap-2 justify-end">
                                                <Button variant="secondary" className="!px-2 !py-1 text-xs" onClick={() => { setSelectedBook(book); setModal('edit'); }}>Edit</Button>
                                                <Button variant="danger" className="!px-2 !py-1 text-xs" onClick={() => setDeletingBook(book)}>Delete</Button>
                                            </div>
                                        </td>
                                    </tr>
                                ))}
            ...
            {(modal === 'add' || (modal === 'edit' && selectedBook)) && (
                <BookFormModal 
                    book={selectedBook}
                    onClose={() => { setModal(null); setSelectedBook(null); }}
                    onSave={handleSave}
                />
            )}
            ...
            {deletingBook && (
                <ConfirmationModal
                    ...
                    message={<>Are you sure you want to delete "<strong>{deletingBook.title}</strong>"? This action cannot be undone.</>}
