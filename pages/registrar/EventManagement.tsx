...
import type { SchoolEvent, UserRole } from '../../types.ts';
...
const EventFormModal: React.FC<{
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (eventToEdit?.id) {
            await apiService.updateSchoolEvent(eventToEdit.id, eventData, user.role);
        } else {
            await apiService.createSchoolEvent(eventData, user.role);
        }
        setIsSaving(false);
        onSave();
    };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
            <Card className="w-full max-w-lg">
                ...
                <div className="flex justify-end gap-2 mt-6 pt-4 border-t">
                    ...
                    <Button variant="secondary" onClick={() => onEdit(event)}>Edit</Button>
                    <Button variant="danger" onClick={() => onDelete(event)}>Delete</Button>
                </div>
            </Card>
        </div>
    );
...
const EventManagement: React.FC = () => {
    ...
    return (
        <div>
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex-grow space-y-1 mt-1">
                                        {dayEvents.map(event => (
                                            <div key={event.id} onClick={(e) => { e.stopPropagation(); setSelectedEvent(event); setModal('detail'); }}
            ...
            {(modal === 'create' || modal === 'edit') && <EventFormModal eventToEdit={selectedEvent} initialDate={selectedDate} onClose={() => setModal(null)} onSave={handleSave} />}
            {modal === 'detail' && selectedEvent && <EventDetailModal event={selectedEvent} onClose={() => setModal(null)} onEdit={(e) => { setSelectedEvent(e); setModal('edit'); }} onDelete={(e) => { setDeletingEvent(e); setModal(null); }} />}
            {deletingEvent && <ConfirmationModal isOpen={!!deletingEvent} onClose={() => setDeletingEvent(null)} onConfirm={handleDelete} title="Delete Event" message={<>Are you sure you want to delete the event "<strong>{deletingEvent.name}</strong>"?</>} />}
        </div>
    );
};
