...
import HostelFormModal from "../../components/modals/HostelFormModal";
import ManageHostelOccupantsModal from "../../components/modals/ManageHostelOccupantsModal";
...
const HostelManagement: React.FC = () => {
  ...
  const [modal, setModal] = useState<
    "create" | "edit" | "manage_occupants" | null
  >(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    const [hostelData, roomData] = await Promise.all([
      ...
      apiService.getAllRoomsByBranch(),
    ]);
  ...
  const calculateOccupancy = (hostelId: string) => {
    ...
    const totalOccupants = hostelRooms.reduce(
      (acc, room) => acc + room.occupantIds.length,
      0
    );
    ...
    <div>
      ...
      {hostels.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {hostels.map((hostel) => {
            const { totalCapacity, totalOccupants } = calculateOccupancy(
              hostel.id
            );
            return (
              <Card key={hostel.id}>
                ...
                <div className="mb-4">
                  ...
                  <div className="w-full bg-slate-200 rounded-full h-2.5">
                    <div
                      ...
                      style={{
                        width: `${
                          (totalOccupants / (totalCapacity || 1)) * 100
                        }%`,
                      }}
                    ></div>
                  </div>
                ...
                <div className="flex justify-end gap-2 mt-4 pt-4 border-t">
                  ...
                  <Button
                    variant="danger"
                    onClick={() => setDeletingHostel(hostel)}
                  >
                    Delete
                  </Button>
                </div>
      ...
      {/* This call is correct. The error lies in the HostelFormModal component itself. */}
      {(modal === "create" || (modal === "edit" && selectedHostel)) &&
        user?.branchId && (
          <HostelFormModal
      ...
      {/* This call is correct. The error lies in the ManageHostelOccupantsModal component itself. */}
        ...
        <ConfirmationModal
          ...
          message={
            <>
              ...
              <strong>{deletingHostel.name}</strong>"? All rooms will be deleted
              and students unassigned.
            </>
          }
