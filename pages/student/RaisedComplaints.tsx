...
const RaisedComplaints: React.FC = () => {
    ...
    const fetchData = async () => {
        ...
        setLoading(true);
        try {
            const data = await apiService.getComplaintsByStudent(user.id);
            setComplaints(data);
        } catch (error) {
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold text-text-primary-dark">My Raised Complaints</h1>
                <Button onClick={() => setIsComplaintModalOpen(true)}>Raise New Complaint</Button>
            </div>
            <Card>
                <p className="mb-6 text-text-secondary-dark">This section lists the complaints you have submitted. You can mark a complaint as resolved if the issue has been addressed to your satisfaction.</p>
                
                {loading ? (
                    <p>Loading complaints...</p>
                ) : complaints.length === 0 ? (
                    <div className="text-center p-8">
                        <p className="text-text-secondary-dark">You have not raised any complaints.</p>
                    </div>
                ) : (
                    <div className="space-y-4">
                        {complaints.map(complaint => (
                            <details key={complaint.id} className="bg-slate-50 p-4 rounded-lg" open={complaint.status === 'Open'}>
                                <summary className="cursor-pointer font-semibold text-text-primary-dark flex justify-between items-center">
                                    <div>
                                        ...
                                        {complaint.teacherName ? 
                                            <span className="font-normal text-text-secondary-dark"> (against {complaint.teacherName})</span> :
                                            <span className="font-normal text-text-secondary-dark"> (General Complaint)</span>
                                        }
                                    </div>
                                ...
                                <div className="mt-4 pt-4 border-t border-slate-200">
                                    ...
                                    {complaint.status === 'Open' && (
                                        <div className="text-right mt-4">
                                            <Button onClick={() => handleMarkResolved(complaint.id)}>Mark as Resolved</Button>
                                        </div>
                                    )}
                                </div>
                            </details>
