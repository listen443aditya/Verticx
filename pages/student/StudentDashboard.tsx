...
const StudentDashboard: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // 2. Extract Principal Data (from branch relation)
      if ((result.branch as any).principal) {
        setPrincipal((result.branch as any).principal);
      }

      // 3. Extract Transport Details (Only if student has opted)
      // The backend sends 'busStop' included in the 'student' object
      ...
      // 4. Extract Hostel Details (Only if student has opted)
      // The backend sends 'room' included in the 'student' object
  ...
  // Note: fees.totalOutstanding already includes these from the backend now
  // We just use these for display in the Fee Card breakdown
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* COLUMN 1: Identity, Fees, Transport, Hostel */}
        <div className="space-y-6">
          {/* Identity Card */}
          <Card>
              ...
              <div className="flex justify-between pt-2 border-t border-slate-200 mt-2">
                ...
                <span
                  className={`font-bold text-lg ${
                    displayOutstanding > 0 ? "text-red-600" : "text-green-600"
                  }`}
                >
          ...
          {/* Transport Card - Visible ONLY if Transport is Opted */}
          {transportDetails && (
            <Card>
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between border-b border-slate-100 pb-2">
                  <span className="text-text-secondary-dark">Route Name:</span>{" "}
                  <span className="font-semibold text-slate-800">
                    {transportDetails.route.routeName}
                  </span>
                </div>
                <div className="flex justify-between border-b border-slate-100 pb-2">
                  <span className="text-text-secondary-dark">Bus Number:</span>{" "}
                  <span className="font-semibold text-slate-800">
                ...
                <div className="p-3 bg-slate-50 rounded-lg border border-slate-100 space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="text-slate-800">
                      {transportDetails.route.driverName}
                    </span>
                  </div>
                  <div className="flex justify-between">
                ...
                <div className="p-3 bg-slate-50 rounded-lg border border-slate-100">
                  <div className="flex justify-between mb-1">
                    <span className="font-bold text-brand-primary">
                      Stop: {transportDetails.stop.name}
                    </span>
                  </div>
                  <div className="flex justify-between text-xs text-slate-500 mt-2">
                    <div className="text-center bg-white px-2 py-1 rounded border border-slate-200">
                    ...
                    <div className="text-center bg-white px-2 py-1 rounded border border-slate-200">
                      <span className="block text-[10px] uppercase">Drop</span>
                      <span className="font-mono font-medium text-slate-800">
                        {transportDetails.stop.dropTime}
                      </span>
                    </div>
                  </div>
                </div>
          ...
          {/* Hostel Card - Visible ONLY if Hostel is Opted */}
          {accommodationDetails && (
            <Card>
              <h3 className="text-lg font-semibold mb-3 flex items-center gap-2">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between border-b border-slate-100 pb-2">
                  <span className="text-text-secondary-dark">Room Number:</span>{" "}
                  <span className="font-mono font-bold text-brand-primary bg-brand-primary/5 px-2 rounded">
                ...
                <div className="p-3 bg-slate-50 rounded-lg border border-slate-100 space-y-2">
                  <div className="flex justify-between">
                    <span className="text-slate-500 font-medium">Warden:</span>
                    <span className="text-slate-800">
