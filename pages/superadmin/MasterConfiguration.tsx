...
// FIX: Corrected import to use the adminApiService instance from the services index.
import { adminApiService as apiService } from '../../services';
...
const MasterConfiguration: React.FC = () => {
    ...
    return (
        <div>
            <h1 className="text-3xl font-bold text-text-primary-dark mb-6">Master Configuration</h1>
            <div className="space-y-6">
                <Card>
                    <h2 className="text-xl font-semibold text-text-primary-dark mb-4">Global ERP Settings</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <Input
                            label="Default ERP Price per Student"
                            type="number"
                            value={String(settings.defaultErpPrice)}
                            onChange={(e) => handleSettingChange('defaultErpPrice', Number(e.target.value))}
                        />
                        <div>
                             <label className="block text-sm font-medium text-text-secondary-dark mb-1">Login Page Announcement</label>
                             <textarea
                                value={settings.loginPageAnnouncement}
                                onChange={(e) => handleSettingChange('loginPageAnnouncement', e.target.value)}
                                rows={3}
                ...
                <Card>
                    <h2 className="text-xl font-semibold text-text-primary-dark mb-4">System-Wide Feature Toggles</h2>
                    <p className="text-sm text-text-secondary-dark mb-4">
                        Enable or disable feature modules across the entire system. Disabling a feature here will prevent school-level admins from enabling it.
                    </p>
                            ...
                            <div key={portalName}>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                                    {Object.entries(portalFeatures).map(([key, label]) => (
                                        <label key={key} className="flex items-center space-x-2 p-2 rounded hover:bg-slate-100 cursor-pointer">
                                            <input 
                                                type="checkbox"
