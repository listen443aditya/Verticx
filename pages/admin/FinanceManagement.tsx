...
const StudentFeeCollections: React.FC<{
  ...
  const [financialData, setFinancialData] =
    useState<SystemWideFinancials | null>(null);
  ...
  const netFlow = isFiltered
    ? summary.totalCollected - summary.totalExpenditure
    : 0;
    ...
    <div className="space-y-6">
      <Card>
        <h2 className="text-xl font-semibold mb-4">
          Student Fee Summary{" "}
          {isFiltered && (
            <span className="text-base font-normal text-text-secondary-dark">
            ...
            <thead className="border-b border-slate-200 text-sm text-text-secondary-dark">
              <tr>
                ...
                <th className="p-4 text-right">
                  {isFiltered ? "Collected (Period)" : "Total Collected"}
                </th>
                <th className="p-4 text-right">
                  {isFiltered ? "Pending (Current)" : "Total Pending"}
                </th>
              ...
              {filteredData.map((school) => (
                ...
                >
                  ...
                  <td className="p-4 text-right font-semibold text-green-600">
                    {school.collected.toLocaleString()}
                  </td>
                  <td className="p-4 text-right font-semibold text-red-600">
                    {school.pending.toLocaleString()}
                  </td>
                  <td className="p-4 text-right">
                    <Button
                      variant="secondary"
                      className="!px-3 !py-1 text-xs"
                      onClick={() => setViewingDetailsOf(school.id)}
                    >
                      View Details
                    </Button>
                  </td>
...
const ErpBillingsDashboard: React.FC = () => {
  ...
  const StatCard: React.FC<{
    ...
    <Card className="flex items-center p-4">
      <div className={`p-3 mr-4 text-white rounded-full ${color}`}>{icon}</div>
      <div>
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <StatCard
          ...
          value={`₹${data.summary.totalBilled.toLocaleString()}`}
        ...
        <StatCard
          ...
          value={`₹${data.summary.pendingAmount.toLocaleString()}`}
          icon={<AlertTriangleIcon className="w-6 h-6" />}
          color="bg-red-500"
        />
        <StatCard
          title="Total Active Schools"
          value={data.summary.totalSchools.toString()}
        ...
        />
        <StatCard
          title="Schools with Dues"
          value={data.summary.pendingSchoolsCount.toString()}
          icon={<AlertTriangleIcon className="w-6 h-6" />}
          color="bg-orange-500"
        />
      </div>
            ...
            <table className="w-full text-left text-sm">
              <thead className="sticky top-0 bg-surface-dark/90 backdrop-blur-sm">
                <tr className="border-b">
                  ...
                  <th className="p-2 text-center">Next Due Date</th>
                  <th className="p-2 text-center">Overdue by</th>
                ...
                {data.billingStatusBySchool.map((school) => (
                  ...
                  >
                    ...
                    <td className="p-2 text-right font-semibold text-red-600">
                      {school.pendingAmount.toLocaleString()}
                    </td>
                    <td className="p-2 text-center">{school.nextDueDate}</td>
                    <td
                      className={`p-2 text-center font-bold ${
                        school.daysOverdue > 0
                          ? "text-red-600"
                          : "text-green-600"
                      }`}
                    >
                      {school.daysOverdue > 0
                        ? `${school.daysOverdue} days`
                        : "On Time"}
                    </td>
...
const FinanceManagement: React.FC = () => {
  const { user } = useAuth();
  const [activeTab, setActiveTab] = useState<"studentFees" | "erpBillings">(
    "studentFees"
  );
  ...
  return (
    <div>
      ...
      <div className="flex border-b border-slate-200 mb-6">
        <button
          className={tabButtonClasses(activeTab === "studentFees")}
        ...
        {user?.role === "SuperAdmin" && (
          <button
            className={tabButtonClasses(activeTab === "erpBillings")}
            onClick={() => setActiveTab("erpBillings")}
          >
            ERP Billings
          </button>
        )}
