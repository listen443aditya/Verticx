...
const StatCard: React.FC<{
  ...
  <div className="bg-slate-100 p-4 rounded-lg text-center">
    <div className="text-brand-secondary mx-auto w-10 h-10 mb-2 flex items-center justify-center">
      {icon}
    </div>
...
const FeatureManagement: React.FC<{
  ...
  onSave: (newFeatures: Record<string, boolean>) => Promise<void>;
}> = ({ initialFeatures, globalFeatures, onSave }) => {
            ...
            {Object.entries(portalFeatures).map(([key, label]) => {
              ...
              return (
                <div
                  ...
                  title={
                    !isGloballyEnabled
                      ? "This feature has been disabled system-wide by the Super Admin and cannot be changed here."
                      : ""
                  }
                >
                  <label
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={!!(features[key] && isGloballyEnabled)}
                    ...
                    <span
                      className={`text-sm ${
                        !isGloballyEnabled ? "text-slate-400 line-through" : ""
                      }`}
                    >
                      {label}
                    </span>
...
const SchoolDetailModal: React.FC<{ branch: Branch; onClose: () => void }> = ({
  branch,
  onClose,
}) => {
  const { user } = useAuth();
  const [details, setDetails] = useState<SchoolDetails | null>(null);
  const [globalSettings, setGlobalSettings] = useState<SystemSettings | null>(
    null
  );
  ...
  const [erpConcessionPercentage, setErpConcessionPercentage] =
    useState<string>("0");
  const [isSavingPrice, setIsSavingPrice] = useState(false);
  const [billingCycle, setBillingCycle] =
    useState<Branch["billingCycle"]>("monthly");
  ...
  const fetchDetails = useCallback(async () => {
    ...
    try {
      ...
      const branchPayments = allPayments
      ...
      setErpConcessionPercentage(
        data?.branch.erpConcessionPercentage?.toString() || "0"
      );
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
            ...
            <div className="lg:col-span-1 space-y-6">
              <Card>
                ...
                <div className="flex items-center gap-4">
                  <img
                    src={
                      details.branch.principalPhotoUrl ||
                  ...
                  <div>
                    ...
                    <Button
                      variant="danger"
                      className="!px-2 !py-1 text-xs mt-2"
              ...
              <Card>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <StatCard
                    title="Students"
                    value={details.students.length.toString()}
                    icon={<StudentsIcon className="w-8 h-8" />}
                  />
                  <StatCard
                    title="Teachers"
                    value={details.teachers.length.toString()}
                    icon={<TeachersIcon className="w-8 h-8" />}
                  />
                  <StatCard
                    title="Classes"
                    value={details.classes.length.toString()}
                    icon={<AcademicsIcon className="w-8 h-8" />}
                  />
                </div>
              </Card>
            </div>
            <div className="lg:col-span-2 space-y-6">
              <Card>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div>
                    <p className="font-medium text-text-secondary-dark">
                      Principal Contact
                    </p>
                  ...
                  <div>
                    ...
                    <p className="font-mono">
                      {details.branch.bankAccountNumber || "N/A"}
                    </p>
                  </div>
                  <div>
                    <p className="font-medium text-text-secondary-dark">
                    ...
                    <p className="font-mono">
                      {details.branch.bankIfscCode || "N/A"}
                    </p>
                  </div>
                </div>
              </Card>
                ...
                <Card>
                  ...
                  >
                    <Input
                      ...
                      type="number"
                      value={erpPrice}
                      onChange={(e) => setErpPrice(e.target.value)}
                    />
                    {user?.role === "SuperAdmin" && (
                      <Input
                        label="Concession (%)"
                        ...
                        onChange={(e) =>
                          setErpConcessionPercentage(e.target.value)
                        }
                      />
                    )}
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setBillingCycle(
                            e.target.value as Branch["billingCycle"]
                          )
                        }
                  ...
                  <div className="flex justify-between items-center mt-4 pt-4 border-t">
                    <p className="text-sm text-text-secondary-dark">
                      Next due date will be: <strong>{displayDueDate}</strong>
                    </p>
                    <Button
                      onClick={handleSaveBillingConfig}
                      disabled={isSavingPrice}
                    >
                      {isSavingPrice ? "Saving..." : "Save Config"}
                    </Button>
                  </div>
                </Card>
              )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              ...
              <ResponsiveContainer width="100%" height={300}>
                <BarChart
                  data={
                    details.subjectPerformanceByClass?.[selectedClassId] || []
                  }
                >
                  <CartesianGrid strokeDasharray="3 3" />
            ...
            <Card className="lg:col-span-1">
              <h3 className="text-lg font-semibold mb-3">
                Top 5 Teacher Performance Index
              </h3>
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b">
                    <th className="p-2">Teacher</th>
                    <th className="p-2 text-right">Performance Index</th>
                  </tr>
                </thead>
                <tbody>
                  {details.teacherPerformance?.map((t) => (
                    ...
                    >
                      ...
                      <td className="p-2 text-right font-semibold text-brand-secondary">
                ...
                {details.topStudents?.map((student) => (
                  ...
                  >
                    <div className="flex items-center">
                      <span
                        className={`font-bold text-lg w-8 text-center text-brand-accent`}
                      >
                        #{student.rank}
                      </span>
                      <div>
              ...
              <table className="w-full text-left text-sm">
                <thead className="sticky top-0 bg-surface-dark/90 backdrop-blur-sm">
                  <tr className="border-b">
                    ...
                    <th className="p-2 text-right">Pending Fees</th>
                    <th className="p-2 text-center">Defaulters</th>
                  </tr>
                </thead>
                <tbody>
                  {details.classFeeDetails.map((c) => (
                    ...
                    >
                      ...
                      <td className="p-2 text-center">{c.studentCount}</td>
                      <td className="p-2 text-right">
                        {c.totalFees.toLocaleString()}
                      </td>
                      <td className="p-2 text-right font-semibold text-red-500">
                        {c.pendingFees.toLocaleString()}
                      </td>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <Card>
              <h3 className="text-lg font-semibold mb-3 flex items-center gap-2">
                <InventoryIcon /> Inventory Summary
              </h3>
      ...
      case "features":
        return (
          <FeatureManagement
            initialFeatures={details.branch.enabledFeatures || {}}
            globalFeatures={globalSettings.globalFeatureToggles}
          ...
          <Card>
            ...
            {paymentHistory.length === 0 ? (
              <p className="text-center p-8 text-text-secondary-dark">
                No payment history found for this school.
              </p>
            ) : (
              <div className="overflow-auto max-h-[75vh]">
                <table className="w-full text-left text-sm">
    ...
    >
      <Card className="w-full max-w-6xl h-[95vh] flex flex-col">
        <div className="flex justify-between items-start mb-2">
          <div>
            <p className="font-mono text-sm text-text-secondary-dark bg-slate-100 px-2 py-1 rounded inline-block">
              {branch.registrationId || "N/A"}
            </p>
            <h2 className="text-3xl font-bold text-text-primary-dark mt-1">
              {branch.name}
            </h2>
        ...
        <div className="border-b border-slate-200 mb-4">
          ...
          <button
            className={tabButtonClasses(activeTab === "performance")}
            onClick={() => setActiveTab("performance")}
          >
            Performance
          </button>
          <button
            className={tabButtonClasses(activeTab === "financials")}
            onClick={() => setActiveTab("financials")}
          >
            Financials
          </button>
          <button
            className={tabButtonClasses(activeTab === "assets")}
            onClick={() => setActiveTab("assets")}
          >
            Assets
          </button>
          <button
            className={tabButtonClasses(activeTab === "features")}
            onClick={() => setActiveTab("features")}
          >
            Features
          </button>
          <button
            className={tabButtonClasses(activeTab === "erp_payments")}
            onClick={() => setActiveTab("erp_payments")}
          >
            ERP Payments
          </button>
        </div>
        ...
        {confirmReset && details?.principal && (
          <ConfirmationModal
            ...
            title="Confirm Principal Password Reset"
            message={
              <>
                Are you sure you want to reset the password for{" "}
                <strong>{details.principal.name}</strong>? A new temporary
