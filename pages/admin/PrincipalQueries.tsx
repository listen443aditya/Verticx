...
const ResolveQueryModal: React.FC<{
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    // FIX: Pass the user's role as the first argument as required by the service.
    await adminApiService.resolvePrincipalQuery(
      user.role,
      query.id,
      adminNotes,
      user.id
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
      <Card className="w-full max-w-lg">
        ...
        <div className="space-y-4 p-4 bg-slate-50 rounded-lg mb-4">
          <p>
            <strong>From:</strong> {query.principalName} ({query.schoolName})
          </p>
          <p>
...
const PrincipalQueries: React.FC = () => {
  // FIX: Add the useAuth hook to get the user object.
  const { user } = useAuth();
  const [queries, setQueries] = useState<PrincipalQuery[]>([]);
  const [loading, setLoading] = useState(true);
  const [view, setView] = useState<"Open" | "Resolved">("Open");
  const [resolvingQuery, setResolvingQuery] = useState<PrincipalQuery | null>(
    null
  );
  const { refreshKey, triggerRefresh } = useDataRefresh();

  const fetchData = useCallback(async () => {
    // FIX: Ensure the user object is available before fetching.
    if (!user) return;
    setLoading(true);
    // FIX: Pass the user's role and the current view status to the API call.
    const data = await adminApiService.getPrincipalQueries(user.role, view);
    setQueries(data);
    setLoading(false);
  }, [user, view]); // FIX: Add user and view to the dependency array.
  ...
  const filteredQueries = useMemo(() => {
    // The backend now handles filtering, so we can just use the data as-is.
    return queries;
  }, [queries]);
    ...
    <div>
      ...
      <Card>
        <div className="flex border-b border-slate-200 mb-4">
          <button
            className={viewButtonClasses(view === "Open")}
            onClick={() => setView("Open")}
          >
            Open
          </button>
          <button
            className={viewButtonClasses(view === "Resolved")}
        ...
        {loading ? (
          ...
          <p className="text-center text-text-secondary-dark p-8">
            No {view.toLowerCase()} queries found.
          </p>
        ) : (
                ...
                <summary className="cursor-pointer font-semibold text-text-primary-dark flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-xs font-normal text-text-secondary-dark">
                      From: {query.principalName} ({query.schoolName}) |
