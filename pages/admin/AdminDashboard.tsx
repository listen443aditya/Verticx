...
const LiveFeedItem: React.FC<{ item: AdminDashboardData["liveFeed"][0] }> = ({
  item,
}) => {
  const icons = {
    alert: (
  ...
  return (
    <div className="flex items-start gap-3 p-3 hover:bg-slate-50 rounded-lg">
      ...
      <div>
        ...
        {/* FIX: Convert timestamp string to a Date object before formatting */}
        <p className="text-xs text-text-secondary-dark">
          {item.school} - {new Date(item.timestamp).toLocaleTimeString()}
        </p>
      </div>
    </div>
  );
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`text-xs font-medium ${classes.scoreText}`}>
                  Health Score: {school.healthScore}
                </p>
              </div>
            </div>
          </div>
...
const AdminDashboard: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setData(null); // Set data to null on error to show error message
    } finally {
      setLoading(false);
    }
  }, [user]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 mb-6">
        <StatCard
          title="Total Schools"
          value={summary.totalSchools.toString()}
        ...
        <StatCard
          ...
          icon={
            ...
            >
              ...
              <path d="m17 12 5 3-5 3v-6Z" />
            </svg>
          }
        />
        {user?.role === "SuperAdmin" ? (
          <StatCard
            title="ERP Revenue (This Month)"
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            <h2 className="text-xl font-semibold text-text-primary-dark mb-4">
              System-Wide Fee Collection Trend
            </h2>
            ...
            <div className="space-y-3 max-h-64 overflow-y-auto pr-2">
              {pendingRequests.requests.map((req) => (
                <div key={req.id} className="p-3 bg-slate-50 rounded-lg">
                  ...
                  <p className="text-xs text-text-secondary-dark">
                    {req.location} - by {req.principalName}
                  </p>
                </div>
              ))}
