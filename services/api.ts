...
api.interceptors.request.use(
  (config) => {
    ...
    const token = localStorage.getItem("token"); // Verify 'token' is the key you use to save the JWT
...
export const suspendStaff = (id: string) =>
  api.put(`/principal/staff/${id}/suspend`);

export const reinstateStaff = (id: string) =>
  api.put(`/principal/staff/${id}/reinstate`);

export const getFinanceOverview = () => api.get("/principal/finance/overview");

export const getAttendanceOverview = () =>
  api.get("/principal/attendance/overview");
