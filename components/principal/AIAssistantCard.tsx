...
const AIAssistantCard: React.FC<AIAssistantCardProps> = ({ data }) => {
  const [messages, setMessages] = useState<Message[]>([
    {
      sender: "ai",
      text: "Hello Principal! I'm your AI Assistant. Ask me anything about your school's data.",
    },
  ]);
  ...
  const constructPrompt = (question: string) => {
    ...
    const context = `
        ...
        **Academic Performance:**
        - Class Performance (Average Score %): ${classPerformance
          .map((c) => `${c.name}: ${c.performance.toFixed(1)}%`)
          .join(", ")}
        - Top 5 Teachers (Performance Index): ${teacherPerformance
          .map((t) => `${t.teacherName}: ${t.performanceIndex.toFixed(1)}/100`)
          .join(", ")}
        - Top 5 Students (School Rank): ${topStudents
          .map((s) => `${s.studentName} (Rank ${s.rank} in ${s.className})`)
          .join(", ")}
        - Syllabus Progress (%): ${syllabusProgress
          .map((s) => `${s.name}: ${s.progress.toFixed(1)}%`)
          .join(", ")}

        **Administrative Data:**
        - Pending Staff Requests: Leave(${
          pendingStaffRequests.leave ?? 0
        }), Attendance Changes(${
      pendingStaffRequests.attendance ?? 0
    }), Fee Template Changes(${pendingStaffRequests.fees ?? 0})
        - Fee Collections by Grade: ${collectionsByGrade
          .map(
            (g) =>
              `${g.name}: ${(g.collected ?? 0).toLocaleString()} collected, ${(
                g.due ?? 0
              ).toLocaleString()} due`
          )
          .join("; ")}
            ...
            <div
              className={`max-w-xs md:max-w-sm rounded-lg px-3 py-2 ${
                msg.sender === "user"
                  ...
                  : "bg-slate-200 text-text-primary-dark"
              }`}
            >
