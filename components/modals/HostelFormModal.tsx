...
const HostelFormModal: React.FC<HostelFormModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.id) {
      ...
      // FIX: The branchId must be part of the hostel object payload, not a separate argument.
      await apiService.createHostel({ ...formData, branchId } as Hostel);
    }
    setIsSaving(false);
    onSave();
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
      <Card className="w-full max-w-3xl max-h-[90vh] flex flex-col">
        <h2 className="text-xl font-bold mb-4">
          {formData.id ? "Edit Hostel" : "Create New Hostel"}
        </h2>
        <form
          id="hostel-form"
          ...
          <div className="pt-4 border-t">
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
              {formData.rooms?.map((room) => (
                <div
                  key={room.id}
                  className="grid grid-cols-12 items-center gap-2 bg-slate-100 p-2 rounded"
                >
                  ...
                  <Button
                    type="button"
                    variant="danger"
                    className="!p-1 h-6 w-6 col-span-1"
        ...
        <div className="flex justify-end gap-4 mt-6 pt-4 border-t">
          ...
          {/* The button now submits the form via the `form` attribute */}
