...
const PayErpBillModal: React.FC<PayErpBillModalProps> = ({ isOpen, onClose, onSuccess, amount, branch, principal }) => {
    ...
    const handlePayment = () => {
        ...
        const options = {
            ...
            handler: async (response: any) => {
                try {
                    await principalApiService.payErpBill(branch.id, amount, response.razorpay_payment_id);
                    onSuccess();
                } catch (apiError) {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
            <Card className="w-full max-w-md">
                ...
                <div className="bg-slate-50 p-4 rounded-lg text-center">
                    ...
                    <p className="text-4xl font-extrabold text-brand-primary">{amount.toLocaleString()}</p>
                </div>
                {error && <p className="text-sm text-red-500 text-center my-4">{error}</p>}
