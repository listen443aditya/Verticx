...
const ManageHostelOccupantsModal: React.FC<ManageHostelOccupantsModalProps> = ({ hostel, branchId, onClose }) => {
    ...
    const unassignedStudents = useMemo(() => allStudents.filter(s => !s.roomId), [allStudents]);
    const availableRooms = useMemo(() => rooms.filter(r => r.occupantIds.length < r.capacity), [rooms]);
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
            <Card className="w-full max-w-4xl max-h-[90vh] flex flex-col">
                 <h2 className="text-xl font-bold text-text-primary-dark mb-4">Manage Occupants for {hostel.name}</h2>
                 {loading ? <p>Loading...</p> : (
                        ...
                        <div className="bg-slate-50 p-4 rounded-lg flex flex-col">
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <select value={selectedRoomId} onChange={e => setSelectedRoomId(e.target.value)} className="w-full bg-white border border-slate-300 rounded-md py-2 px-3">
                                        <option value="">-- Select Available Room --</option>
                                        {availableRooms.map(r => <option key={r.id} value={r.id}>Room {r.roomNumber} ({r.occupantIds.length}/{r.capacity})</option>)}
                                    </select>
                                </div>
                                <Button onClick={handleAssignStudent} disabled={isActionLoading || !selectedStudent || !selectedRoomId}>
                            ...
                            <div className="overflow-y-auto flex-grow space-y-2 pr-2">
                               {rooms.map(room => (
                                    <div key={room.id} className="bg-white p-2 rounded shadow-sm">
                                        <p className="font-medium border-b pb-1 mb-1">Room {room.roomNumber} ({room.occupantIds.length}/{room.capacity})</p>
                                        {room.occupantIds.length > 0 ? room.occupantIds.map(studentId => {
                                            const student = allStudents.find(s => s.id === studentId);
                                            return (
                                                <div key={studentId} className="flex justify-between items-center text-sm py-1">
                                                    <span>{student?.name || 'Unknown Student'}</span>
                                                    <Button variant="danger" className="!px-2 !py-1 text-xs" onClick={() => handleRemoveStudent(studentId)} disabled={isActionLoading}>Remove</Button>
                                                </div>
                                            )
