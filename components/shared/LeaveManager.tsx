...
import { sharedApiService as apiService } from '../../services';
import type { LeaveApplication, LeaveType, User, LeaveSetting } from '../../types.ts';
import Card from '../ui/Card.tsx';
import Button from '../ui/Button.tsx';
import Input from '../ui/Input.tsx';
import { useDataRefresh } from '../../contexts/DataRefreshContext.tsx';

const LeaveBalanceCard: React.FC<{ type: string; remaining: number; total: number }> = ({ type, remaining, total }) => {
    ...
    return (
        <Card className="text-center">
            ...
            <p className="text-4xl font-bold my-2 text-brand-secondary">{remaining}<span className="text-xl text-text-secondary-dark">/{total}</span></p>
...
const LeaveManager: React.FC<LeaveManagerProps> = ({ user }) => {
    ...
    const [leaveBalances, setLeaveBalances] = useState<Record<string, number>>({}); // Remaining
    ...
    const fetchLeaveData = useCallback(async () => {
        ...
        const [appData, branchSettings, freshUser] = await Promise.all([
            ...
            apiService.getLeaveSettingsForBranch(user.branchId),
            apiService.getUserById(user.id) // Fetch latest user data for balances
        ]);
        setApplications(appData);

        const userRoleSetting = branchSettings.find(s => s.role === user.role);
        
        if (userRoleSetting?.settings) {
            const totals = userRoleSetting.settings as Record<string, number>;
            const types = Object.keys(totals).filter(k => totals[k] > 0) as LeaveType[];
            
            const normalizedTotals: Record<string, number> = {};
            types.forEach(t => { normalizedTotals[t.toLowerCase()] = totals[t]; });
                    ...
                    <h2 className="text-xl font-semibold mb-4">Apply for Leave</h2>
                     <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            ...
                            <select value={leaveType} onChange={e => setLeaveType(e.target.value as LeaveType)} className="w-full bg-white border border-slate-300 rounded-md py-2 px-3 mt-1">
                                {availableLeaveTypes.map(lt => <option key={lt} value={lt}>{lt}</option>)}
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea value={reason} onChange={e => setReason(e.target.value)} rows={4} className="w-full bg-white border border-slate-300 rounded-md py-2 px-3 mt-1" required />
                        </div>
                            ...
                            <div key={app.id} className="bg-slate-50 p-3 rounded-lg">
                                <div className="flex justify-between items-start">
                                    <div>
                                        ...
                                        <p className="text-xs text-text-secondary-dark">{app.startDate} to {app.endDate}</p>
                                    </div>
