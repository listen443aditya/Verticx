...
const StaffAttendanceCalendar: React.FC<StaffAttendanceCalendarProps> = ({
  ...
  useEffect(() => {
    const fetchStaff = async () => {
      ...
      // 3. CREATE the cache-busting config
    ...
    fetchStaff();
  }, [apiService, refreshKey]); // 5. ADD refreshKey to dependency array
  ...
  const fetchCalendarData = useCallback(async () => {
    ...
    // 6. CREATE the cache-busting config
    const cacheBustConfig = { params: { _cacheBust: refreshKey } };
    ...
    try {
      ...
      (attendance || []).forEach((rec: any) => {
        // Use rec.date (from StaffAttendanceRecord or TeacherAttendanceRecord)
  ...
  useEffect(() => {
    fetchCalendarData();
  }, [fetchCalendarData]); // 'refreshKey' is now a dependency
